 ...
 * Route: /medicare-supplement-plans/compare
...
function CoverageCell({ value }: { value: CoverageValue }) {
  if (value === "✓") return <CheckCircle2 className="w-5 h-5 text-green-600 mx-auto" aria-hidden="true" />;
  if (value === "✗") return <XCircle className="w-5 h-5 text-red-400 mx-auto" aria-hidden="true" />;
        ...
        <div className="container relative z-10">
          ...
          <div className="flex flex-wrap gap-4">
            <a href="tel:+18883358996"  data-invoca-phone-number="18883358996"
              onClick={() => trackPhoneClick({ phone_number: "(888) 335-8996", page_section: "compare" })} className="invoca-phone inline-flex items-center gap-2 bg-teal-600 hover:bg-teal-500 text-white font-semibold px-6 py-3 rounded-lg transition-colors">
              <Phone className="w-4 h-4" aria-hidden="true" /> Get Free Quotes
            </a>
          </div>
        </div>
      </section>
                ...
                <tr className="bg-slate-900 text-white">
                  ...
                  {PLANS.map((p) => (
                    <th key={p} className="px-3 py-3 text-center font-bold min-w-[60px]">
                      <Link href={`/medicare-supplement-plans/plan-${p.toLowerCase()}`} className="hover:text-teal-300 transition-colors">
                        {p}
                      </Link>
                    </th>
                  ))}
                    ...
                    {row.note && (
                      <tr key={`${i}-note`} className={i % 2 === 0 ? "bg-white" : "bg-slate-50"}>
                        <td colSpan={PLANS.length + 1} className="px-4 pb-3 text-xs text-slate-500 italic">
                          <Info className="w-3 h-3 inline mr-1" aria-hidden="true" />{row.note}
                        </td>
                      </tr>
                    )}
                  </>
