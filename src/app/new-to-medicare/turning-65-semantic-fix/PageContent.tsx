...
/**
 * Turning 65 Timeline Page — SEMANTIC FIX VERSION
 * Same visual appearance as the original, but with proper semantic HTML:
 * - <ol><li> for numbered step sequences (timeline tasks)
 * - <ul><li> for unordered lists (after-enrollment checklist)
 * - <dl><dt><dd> for penalty definitions
 * - <nav aria-label="Breadcrumb"> with <ol> for breadcrumbs
 * - <nav> with <ul><li> for sidebar TOC
 * - <article> wrapper, <header> for hero
 * - <aside> for tips/callouts
...
export default function Turning65Semantic() {
  ...
  return (
    <article className="min-h-screen bg-white">
      {/* Hero */}
      <header className="bg-gradient-to-br from-slate-900 via-slate-800 to-teal-900 text-white">
        <div className="max-w-6xl mx-auto px-4 py-16 md:py-24">
          ...
          <nav aria-label="Breadcrumb" className="mb-8">
            <ol className="flex items-center gap-2 text-sm text-slate-300">
              ...
              <li><Link href="/medicare-101" className="hover:text-white transition-colors">New to Medicare</Link></li>
              <li aria-hidden="true"><ChevronRight className="w-3.5 h-3.5" /></li>
              <li aria-current="page"><span className="text-teal-300">Turning 65 Timeline</span></li>
            </ol>
          </nav>
      ...
      <div className="max-w-6xl mx-auto px-4 py-12 md:py-16">
        <div className="flex gap-12">
          {/* Sticky TOC Sidebar — now uses ul/li */}
          <aside className="hidden lg:block w-56 shrink-0 order-last" aria-label="Page navigation">
            <div className="sticky top-24">
              ...
              <aside className="mt-8 p-4 bg-teal-50 rounded-xl border border-teal-100" aria-label="Contact information">
          ...
          <main className="flex-1 min-w-0 order-first">
            ...
            <section id="overview" className="mb-16 scroll-mt-24">
              ...
              {/* Visual IEP diagram */}
              <figure className="bg-slate-50 rounded-xl border border-slate-200 p-6 mb-8">
                <figcaption className="text-lg font-bold text-slate-900 mb-4">Your 7-Month Enrollment Window</figcaption>
                <div className="flex items-center gap-1 overflow-x-auto pb-2" role="img" aria-label="7-month IEP timeline showing best enrollment months are the first 3 months before your birthday">
            ...
            {/* Timeline Milestones — using <ol> for ordered steps */}
            {timelineMilestones.map((milestone, idx) => (
              ...
              >
                ...
                {/* Semantic ordered list for numbered steps */}
                <ol className="space-y-4 ml-0 md:ml-16 list-none counter-reset-[step]">
                  {milestone.tasks.map((task, i) => (
                    ...
                    >
                      <div className="flex items-start gap-3">
                        <div className={`w-7 h-7 rounded-full ${milestone.color} flex items-center justify-center shrink-0 mt-0.5`} aria-hidden="true">
                          <span className="text-white text-xs font-bold">{i + 1}</span>
                        </div>
                        <div>
                          <h3 className="font-bold text-slate-900 mb-1"><strong>{task.title}</strong></h3>
            ...
            <section id="iep" className="mb-16 scroll-mt-24">
              ...
              <p className="text-lg text-slate-600 mb-8 leading-relaxed">
                When you enroll during your <strong>IEP</strong> determines when your coverage starts. Enrolling early is always better — here's a month-by-month breakdown:
              </p>

              <div className="overflow-x-auto rounded-xl border border-slate-200">
                <table className="w-full text-sm">
                  <caption className="sr-only">Initial Enrollment Period timeline showing when coverage starts based on enrollment month</caption>
                  <thead>
                    <tr className="bg-slate-50">
                      <th scope="col" className="text-left p-4 font-semibold text-slate-700">IEP Month</th>
                      <th scope="col" className="text-left p-4 font-semibold text-slate-700">Coverage Starts</th>
                      <th scope="col" className="text-center p-4 font-semibold text-slate-700">Recommended?</th>
                    </tr>
                  </thead>
                  <tbody>
                    {iepChart.map((row) => (
            ...
            {/* After Enrollment — using <ul> for checklist */}
            ...
            {/* Penalties — using <dl> for definitions */}
                ...
                {penalties.map((penalty) => (
                  <div key={penalty.part} className="bg-red-50 border border-red-200 rounded-xl p-6">
                    <dt className="flex items-center gap-2 mb-3">
                      ...
                      <strong className="text-lg font-bold text-red-900">{penalty.part}</strong>
                      {penalty.permanent && (
