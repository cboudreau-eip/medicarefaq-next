      ...
      <div className="bg-white border-[3px] border-[#2b2b2b] overflow-hidden transition-all duration-300 group-hover:shadow-[8px_8px_0_#2b2b2b] group-hover:-translate-y-1.5 shadow-[5px_5px_0_#2b2b2b]" style={{ borderRadius: "255px 15px 225px 15px / 15px 225px 15px 255px" }}>
        <div className="relative h-[160px] overflow-hidden bg-gray-100">
          ...
          <div className="absolute top-3 left-3 flex items-center gap-2">
            <span className="inline-block text-xs font-bold px-2.5 py-1 border-[2px] border-[#2b2b2b] bg-[#ffdd57] text-[#2b2b2b]" style={{ borderRadius: "120px 8px 120px 8px / 8px 120px 8px 120px", transform: "rotate(-2deg)" }}>Blog</span>
...
export default function BlogArticlesPage() {
  ...
  const { articles, loading, error, blogCount, faqCount, totalCount, refresh } = useArticles({ authenticated, authFetch, typeFilter: "blog" });
  ...
  return (
    <SketchLayout totalCount={totalCount} blogCount={blogCount} faqCount={faqCount} onLogout={logout}>
      <div className="sticky top-0 z-10 px-8 py-4 flex items-center justify-between" style={{ borderBottom: "3px dashed #2b2b2b" }}>
        <div className="flex items-center gap-4">
          <h1 className="text-3xl font-bold text-[#2b2b2b]" style={{ fontFamily: "'Caveat', cursive" }}>
            <span style={{ background: "linear-gradient(180deg, transparent 55%, rgba(255,221,87,0.75) 55%)", padding: "0 4px" }}>Blog Articles</span>
          </h1>
      ...
      <div className="flex-1 overflow-y-auto p-8">
        ...
        {!loading && !error && filtered.length === 0 && <div className="flex flex-col items-center justify-center py-24 text-[#888888]"><FileText className="w-12 h-12 mb-3 opacity-30" /><p className="text-lg text-[#555555]">No blog articles found</p></div>}
